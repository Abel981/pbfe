...
function AgentsDisplay({ country,state,city,gender,agentType,phone }) {
    ...
    useEffect(() => {
        ...
        const query = queryString.stringify({ country, state, city, gender,agentType,phone });
        console.log(query)
      ...
      console.log(query)
//      
        axios.get(`https://localhost:3000/admin/resource/agents?${query}`, {withCredentials: true})
          .then(res => res.data)
    ...
    return (
      <div className='flex flex-col justify-center items-center'>
        ...
        <Grid container spacing={2}>
          ...
          {agents.map(agent => (
            <Grid item xs={12} sm={6} md={4} key={agent.id} style={{ boxShadow: '0px 1px 3px rgba(0, 0, 0, 0.2)' }}>
            <ListItem
            key={agent.id}
            component="button"
         ...
         <Modal open={!!selectedAgent} onClose={handleCloseModal}
          ...
          <CardContent>
            ...
            {selectedAgent && (
              <div style={{overflow: 'auto'}}>
                <h2 className='mb-3'>Full Name: {`${selectedAgent.firstName} ${selectedAgent.middleName} ${selectedAgent.lastName}`}</h2>
                ...
                {selectedAgent.agentType == 'detailed' &&
                    <p className='mb-3'>tradeUrl: {selectedAgent.tradeUrl}</p>
                      ...
                      <CardActions>
        <Button size="small" onClick={() => setSelectedAgent(null)}>OK</Button>
      </CardActions>
          </CardContent>
